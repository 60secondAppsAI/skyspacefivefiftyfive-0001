        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Itinerarys</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalItinerarys = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalItinerarys">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Itinerary</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ItineraryId" type="text" placeholder="Enter ItineraryId" v-model="itineraryToAdd.itineraryId"></base-input>
  <base-input label="SeatNumber" type="text" placeholder="Enter SeatNumber" v-model="itineraryToAdd.seatNumber"></base-input>
  <base-input label="ClassType" type="text" placeholder="Enter ClassType" v-model="itineraryToAdd.classType"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="itinerarys" :row-key="record => record.ItineraryId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ItineraryPictureView :itinerarys="itinerarys" />
          </div>
...
import ItineraryPictureView from './ItineraryPictureView.vue';


const itinerarysColumns = [
  "itineraryId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    ItineraryPictureView
  },

  data() {
    return {
      modalItinerarys: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'itineraryId',
          visible: true,
          scopedSlots: { customRender: 'itineraryId' },
          sorter: true
          //sorter: (a, b) => a.itineraryId - b.itineraryId,
          //sorter: (a, b) => a.itineraryId.localeCompare(b.itineraryId),
        },
        {
          title: 'Seat Number',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} itinerarys`,
      },
      ...
      itinerarysTable: {
        columns: [...itinerarysColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'itineraryId',           // Column to sort by
  ...
  methods: {
    ...
    async renderItinerarysTable() {
      ...
      let itinerarysTableData = [];
      for (let i = 0; i < this.itinerarys.length; i++) {
        itinerarysTableData.push({
          id: i,
          itineraryId: this.itinerarys[i].itineraryId,
          year: this.itinerarys[i].year,
          date: this.itinerarys[i].date,
          competitionId: this.itinerarys[i].competitionId,
          itineraryId: this.itinerarys[i].itineraryId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-itinerarys',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForItinerarysChanged', this.searchQuery);
		//this.renderItinerarysTable();
    },
	...
	async handleAddSubmitted() {
      this.modalItinerarys = false;

      const currentDate = new Date().getTime();
      this.itineraryToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.itineraryToAdd);
      console.log(jsonData);
      
      const res = await ItineraryService.addItinerary(jsonData);
