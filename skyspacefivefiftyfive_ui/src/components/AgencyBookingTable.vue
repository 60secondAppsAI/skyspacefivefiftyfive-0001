        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Agency Bookings</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalAgencyBookings = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAgencyBookings">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add AgencyBooking</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AgencyBookingId" type="text" placeholder="Enter AgencyBookingId" v-model="agencyBookingToAdd.agencyBookingId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="agencyBookings" :row-key="record => record.AgencyBookingId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AgencyBookingPictureView :agencyBookings="agencyBookings" />
          </div>
...
import AgencyBookingPictureView from './AgencyBookingPictureView.vue';


const agencyBookingsColumns = [
  "agencyBookingId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    AgencyBookingPictureView
  },

  data() {
    return {
      modalAgencyBookings: false,
        isTableView: true,

      columns: [
        {
          title: 'Agency Booking Id',
		dataIndex: 'agencyBookingId',
          visible: true,
          scopedSlots: { customRender: 'agencyBookingId' },
          sorter: true
          //sorter: (a, b) => a.agencyBookingId - b.agencyBookingId,
          //sorter: (a, b) => a.agencyBookingId.localeCompare(b.agencyBookingId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} agencyBookings`,
      },
      ...
      agencyBookingsTable: {
        columns: [...agencyBookingsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'agencyBookingId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAgencyBookingsTable() {
      ...
      let agencyBookingsTableData = [];
      for (let i = 0; i < this.agencyBookings.length; i++) {
        agencyBookingsTableData.push({
          id: i,
          agencyBookingId: this.agencyBookings[i].agencyBookingId,
          year: this.agencyBookings[i].year,
          date: this.agencyBookings[i].date,
          competitionId: this.agencyBookings[i].competitionId,
          agencyBookingId: this.agencyBookings[i].agencyBookingId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-agency-bookings',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAgencyBookingsChanged', this.searchQuery);
		//this.renderAgencyBookingsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalAgencyBookings = false;

      const currentDate = new Date().getTime();
      this.agencyBookingToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.agencyBookingToAdd);
      console.log(jsonData);
      
      const res = await AgencyBookingService.addAgencyBooking(jsonData);
