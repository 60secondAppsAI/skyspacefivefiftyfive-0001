        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Policys</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalPolicys = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPolicys">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Policy</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="PolicyId" type="text" placeholder="Enter PolicyId" v-model="policyToAdd.policyId"></base-input>
  <base-input label="PolicyNumber" type="text" placeholder="Enter PolicyNumber" v-model="policyToAdd.policyNumber"></base-input>
  <base-input label="Details" type="text" placeholder="Enter Details" v-model="policyToAdd.details"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="policys" :row-key="record => record.PolicyId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PolicyPictureView :policys="policys" />
          </div>
...
import PolicyPictureView from './PolicyPictureView.vue';


const policysColumns = [
  "policyId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'policyId',
          visible: true,
          scopedSlots: { customRender: 'policyId' },
          sorter: true
          //sorter: (a, b) => a.policyId - b.policyId,
		...
		dataIndex: 'policyNumber',
          visible: true,
          scopedSlots: { customRender: 'policyNumber' },
          sorter: true
          //sorter: (a, b) => a.policyNumber - b.policyNumber,
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} policys`,
      },
      ...
      policysTable: {
        columns: [...policysColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'policyId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPolicysTable() {
      ...
      let policysTableData = [];
      for (let i = 0; i < this.policys.length; i++) {
        policysTableData.push({
          id: i,
          policyId: this.policys[i].policyId,
          year: this.policys[i].year,
          date: this.policys[i].date,
          competitionId: this.policys[i].competitionId,
          policyId: this.policys[i].policyId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-policys',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPolicysChanged', this.searchQuery);
		//this.renderPolicysTable();
    },
	...
	async handleAddSubmitted() {
      this.modalPolicys = false;

      const currentDate = new Date().getTime();
      this.policyToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.policyToAdd);
      console.log(jsonData);
      
      const res = await PolicyService.addPolicy(jsonData);
