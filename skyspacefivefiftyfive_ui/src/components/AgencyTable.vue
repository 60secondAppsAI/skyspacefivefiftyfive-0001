        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Agencys</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalAgencys = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAgencys">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Agency</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AgencyId" type="text" placeholder="Enter AgencyId" v-model="agencyToAdd.agencyId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="agencyToAdd.name"></base-input>
  <base-input label="ContactInformation" type="text" placeholder="Enter ContactInformation" v-model="agencyToAdd.contactInformation"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="agencys" :row-key="record => record.AgencyId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AgencyPictureView :agencys="agencys" />
          </div>
...
import AgencyPictureView from './AgencyPictureView.vue';


const agencysColumns = [
  "agencyId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'agencyId',
          visible: true,
          scopedSlots: { customRender: 'agencyId' },
          sorter: true
          //sorter: (a, b) => a.agencyId - b.agencyId,
          //sorter: (a, b) => a.agencyId.localeCompare(b.agencyId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} agencys`,
      },
      ...
      agencysTable: {
        columns: [...agencysColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'agencyId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAgencysTable() {
      ...
      let agencysTableData = [];
      for (let i = 0; i < this.agencys.length; i++) {
        agencysTableData.push({
          id: i,
          agencyId: this.agencys[i].agencyId,
          year: this.agencys[i].year,
          date: this.agencys[i].date,
          competitionId: this.agencys[i].competitionId,
          agencyId: this.agencys[i].agencyId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-agencys',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAgencysChanged', this.searchQuery);
		//this.renderAgencysTable();
    },
	...
	async handleAddSubmitted() {
      this.modalAgencys = false;

      const currentDate = new Date().getTime();
      this.agencyToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.agencyToAdd);
      console.log(jsonData);
      
      const res = await AgencyService.addAgency(jsonData);
